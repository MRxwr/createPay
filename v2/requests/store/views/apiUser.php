    ...
    if( $_GET["action"] == "register" ){
        ...
        if( !isset($data["registrationType"]) || empty($data["registrationType"]) ){
            ...
            if( $user = selectDBNew("users",[$data["registrationToken"]],"`registrationToken` = ?","") ){
                ...
                echo outputError(array("msg" => errorResponse($lang,"Registration token not found","كود التسجيل غير موجود")));die();
            }
        ...
        if( $user = selectDBNew("users",[$token],"`keepMeAlive` = ?","") ){
            if( $notifications = selectDB2New("`date`, `title`, `body`, `image`, `status`","notifications",[$user[0]["id"]],"`userId` = ?","") ){
